...
const generateOutline = async (req, res) => {
  try {
    ...
    if (startIndex === -1 || endIndex === -1) {
      ...
      return res
        .status(500)
        .json({ message: "failed to parse ai response, np json array found." });
    }
    ...
    console.error("error generating coutline:", error);
    res
      .status(500)
...
// @desc    Generate content for a chapters
...
// dummy prompt for generateOutline
...
Requirements:
1. Generate exactly sirumChapters || 5) chapters
...
// dummy prompt for generateChapterContent
